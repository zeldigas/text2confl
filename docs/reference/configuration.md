...
| remove-orphans                    | `--remove-orphans`                         |                    | Whether orphaned pages need to be cleaned up in some way. By default all "managed" pages are removed. You can find more about this on [dedicated page](../how-to/manage-orphaned-pages.md)                                                                                                 |
...
| tenant                            | `--tenant`                                 |                    | Tenant id for uploaded pages. You can find more info on [page about multi-tenancy](../how-to/configure-multi-tenancy.md).                                                                                                                                                                    | 
...
| auto-fix-content-tags            | `--auto-fix-content`                       |                    | Whether to automatically fix/massage converted HTML content to correct misnested or unclosed tags (uses Jsoup). Disabled by default. When enabled, converted HTML will be normalized before validation and further processing.                                                         |
...
| code-blocks.collapse-with-expand  |                                            |                    | Use expand macro to put code block inside. Useful for Confluence Cloud, where code block does not have collapse feature, enabled by default for v2 editor, if not specified explicitly.                                                                                             |
| markdown                          |                                            |                    | Section with markdown specific parameters                                                                                                                                                                                                                                           | 
| client.skip-ssl                   |                                            |                    | Skip SSL certificate verification. `false` by default                                                                                                                                                                                                                               | 
| client.requests-per-seconds       |                                            |                    | Activates requests rate limiting when provided. Use if requests rate is throttled by server                                                                                                                                                                                         | 
| client.retry-on-too-many-requests |                                            |                    | Retry requests when server returns 429 or 503 status codes. `true` by default                                                                                                                                                                                                       | 
...
| bundled-macros       | `true`        | Enables [asciidoctor macros for Confluence](./asciidoc/confluence-specific.adoc)                                             |
...
See [Change detection strategies](../explanation/change-detection.md) for a full explanation of the `hash` and `content` strategies and when to use each.

### Autogenerated note

See [Change detection strategies](../explanation/change-detection.md#autogenerated-note) for the default note template and how to customize it with `__doc-root__` and `__file__` variables.
...
| `--requests-per-second`                                |                           | Limits requests per second text2confl will make to Confluence APIs                  |
| `--dry`/`--no-dry`                                     |                           | Activate (or explicitly deactivate) dry run mode                                    |
| `--auto-fix-content`                                   |                           | Enable automatic fixing of invalid or misnested HTML tags in converted content (opt-in). Off by default. |

[simple_macros]: ./markdown/confluence-specific.md#confluence-macros-with-simple-options