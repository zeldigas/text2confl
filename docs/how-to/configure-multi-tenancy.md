...
1. Pages without a tenant are "vacant" and can be claimed by any tenant -
   except [virtual pages](../explanation/virtual-pages.md), which are never
   modified.
2. A page belonging to another tenant cannot be updated; text2confl will report an error.
3. Managed-page cleanup (`remove-orphans: managed`) skips pages that belong to a different tenant.
   The `all` mode removes pages regardless of tenant.

## Recommendations

1. **Shared pages** (e.g. a space home page) can be uploaded without a tenant so all teams can read them without
   conflict.
