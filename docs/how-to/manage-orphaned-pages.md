...
tree. text2confl calls these **orphaned pages** and can clean them up automatically.

## Cleanup modes

There are three cleanup modes, controlled by the `remove-orphans` configuration option or the `--remove-orphans` CLI
flag:
...
A page is **managed** if its content was last written by text2confl. text2confl stores a marker in the page metadata
during upload. Pages created manually and never edited by text2confl in Confluence are not considered managed.
...
When multi-tenancy is configured, managed-page cleanup (`managed` mode) skips pages that belong to a different tenant.
The `all` mode still removes pages regardless of tenant.
